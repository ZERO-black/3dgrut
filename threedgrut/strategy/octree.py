...
class OctreeStrategy(GSStrategy):
    ...
    #                 iteration: int,
    #                 grads: torch.Tensor,    # [N]
    #                 threshold: float,
    #                 extra_ratio: float,
    #                 extra_up: float,
    #                 overlap: bool):
        
    #     init_length = self.model.num_gaussians
    #     grads[~self.anchor_mask] = 0.0
@torch.cuda.nvtx.range("grow_anchor")
def grow_anchor(self,
                iteration: int,
                anchor_grads: torch.Tensor,    # [N]
                extra_up: float,
                overlap: bool):
    ...
    for cur_level in range(self.model.max_level):
        # 2) 현재 레벨 앵커
        level_mask = (self.get_level.squeeze(1) == cur_level)
        ...
        # ------------------------------------------------------------
        # 8) 같은 레벨 후보 앵커 좌표 뽑기
        cand_same_coords = self.get_anchor()[candidate_same_level]  # [M1,3]

        # 9) 하위 레벨 후보 앵커 좌표 뽑기
        cand_down_coords = self.get_anchor()[candidate_down_level]  # [M2,3]

        # ------------------------------------------------------------
        # 10) grid cell 계산 및 중복 처리 + weed_out (same 레벨)
        grid_coords = torch.round(
            (self.get_anchor()[level_mask] - self.model.init_pos) / cur_size - self.model.padding
        ).int()  # [num_level,3]
        ...
        level_ds_mask = (self.get_level.squeeze(1) == (cur_level + 1))
        grid_coords_ds = torch.round(
            (self.get_anchor[level_ds_mask] - self.model.init_pos) / ds_size - self.model.padding
        ).int()  # [num_level+1,3]
        ...
        new_features = self.get_features()[combined_mask]               # [M_new, F, 3]
        new_features_albedo   = new_features[:, 0:1, :]   # [M_new,1,3]
